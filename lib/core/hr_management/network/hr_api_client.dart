...
class HrApiClient {
  ...
  /// Manager dashboard aggregates (`POST /api/hr/dashboard`).
  Future<HrApiEnvelope<Map<String, dynamic>>> fetchDashboard({
      ...
      return HrApiEnvelope<Map<String, dynamic>>(
        ...
        data: {
          'kpis': {
            ...
            'pending': 24,
            'approved': 58,
            'avg_approval_days': 2.4,
          },
          'by_type': [
            {
              'filter_type': 'Annual Leave',
              'label': 'Annual',
              'count': 42,
            },
            {
              'filter_type': 'Sick Leave',
              'label': 'Sick',
              'count': 14,
            },
          ],
          ...
          'top_requesters': [
            {'name': 'Aisha Al-Mansoori', 'count': 4},
            {'name': 'Omar Hassan', 'count': 3},
          ],
          'by_department': [
            {'label': 'R&D', 'share': 0.42},
            {'label': 'Delivery', 'share': 0.28},
          ],
    ...
    try {
      return await _postMap('/api/hr/dashboard', {'period': period});
    } on DioException catch (e) {
      if (e.response?.statusCode == 404) {
        return const HrApiEnvelope<Map<String, dynamic>>(
          success: false,
          error: 'Dashboard API not deployed',
        );
      }
      return HrApiEnvelope<Map<String, dynamic>>(
        success: false,
